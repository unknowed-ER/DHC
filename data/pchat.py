...
class PchatDatasetReader(DatasetReader):
    iterator_shapes = {
        "context": _matrix(),
        "response": _matrix(),
    ...
    def __init__(self,
        ...
        self._datapath = os.path.join(self._cache_dir, 'pchat')
    ...
    def _read(self, mode: str, batch_size: int) -> tf.data.Dataset:
        ...
        drop_remainder = False if mode == 'train' else True # batchsize根据episode确定，为1，不会drop
    ...
    def _load_and_preprocess_all(self, mode: str):
        ...
        if os.path.exists(self._get_preprocessed_fname(mode)):
            ...
            colorlog.info(f"Load cached pchat from {episodes_fname}")
        ...
        for filename in tqdm(filenames[:1000] if mode=='valid' or mode=='test' else filenames[:30000], desc='- Collecting & Tokenizing Sentence'):
            ...
            with open(os.path.join(path, filename), 'r')as f:
                checked_sentences = [tokenize('null')]
                lines = f.readlines()
                if mode == 'train':
                    lines = lines[:-4]
                elif mode == 'valid':
                    checked_sentences += [tokenize(i.strip().split('\t')[3]) for i in lines[:-4]]
                    lines = lines[-4:-2]
                elif mode == 'test':
                    checked_sentences += [tokenize(i.strip().split('\t')[3]) for i in lines[:-2]]
                    lines = lines[-2:]
