...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def split_masks(self, x, length):
        x = tf.expand_dims(x, 1)
        # x = tf.tile(x, (1, self.num_heads, 1))
        x = tf.expand_dims(x, -1)
        x = tf.tile(x, (1, self.num_heads, 1, length))
        return x
    ...
    def call(self, inputs):
        ...
        mask = self.split_masks(mask, length_size)
