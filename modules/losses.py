...
def soft_crossentropy_distillation(prior, posterior, masking):
    # XXX : use softplus? rather than epsilon?
    my_print("soft_crossentropy_distillation")
    with tf.name_scope("soft_crossentropy_distillation"):
        if masking.dtype == tf.bool:
            masking = tf.cast(masking, tf.float32) 
        soft_onehot_answer = tf.stop_gradient(posterior) * masking

        loss = tf.keras.losses.categorical_crossentropy(
            soft_onehot_answer, prior, from_logits=False, label_smoothing=0)
