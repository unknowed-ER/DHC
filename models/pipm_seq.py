...
@add_argument("--sks_type", type=str, default="iclr")   # not used
...
@add_argument("--prediction_bow_loss_type", type=str, default="y")    # not used
@add_argument("--prediction_loss_alpha", type=float, default=0.5)
class PIPM_seq(BaseModel):
    def __init__(self,
                 hparams,
                 vocabulary):
        super().__init__(hparams, vocabulary, "PIPM_seq")
        ...
        if hparams.use_pipm:
            ...
            new_hparams.use_copy_decoder = False
            new_hparams.num_layers = 3
            fst_decoder = TransformerDecoder(new_hparams, vocabulary, self._embedding, self._output_embedding)
            self.prediction_layer = PredictiveLayer(hparams, embedding=self._embedding, decoder=fst_decoder, my_scope="prediction")
    ...
    def call(self, inputs, training: bool = True):
        ...
        if self.hparams.use_pipm:
            # call(self, Qprior, Xvec, Kvec, Knum, Xtensor, Xidx, Ytensor, Yidx, training=True):
            prior_dialog_outputs, pred_logits, pred_vecs = self.prediction_layer(prior_dialog_outputs, context_output, pooled_knowledge_embeddings, num_knowledges, context_outputs, context, response_outputs, response, training)
            # prediction_bow_loss = get_predictive_bow_loss(self.hparams, pred_logits, context, response, knowledge_sentences[:,0])
            # tf.print(tf.shape(prior_dialog_outputs), tf.shape(pred_logits), tf.shape(pred_vecs))
            prediction_bow_loss = self.gene_loss_func(logits=pred_logits, response=response, response_length=response_length, training=training)
    ...
    def gene_loss_func(self, logits, response, response_length, training):  
        """need to use with 
           gen_loss = tf.reduce_sum(tf.reshape(gen_loss, [batch_size, max_episode_length]), axis=1) / tf.cast(episode_length, tf.float32)
         """
        response_mask = tf.sequence_mask(response_length, dtype=tf.float32)
        if training:    # [epbsz, maxlenK+ctxlen] & [epbsz, maxlenK+ctxlen, 768hdd] & [epbsz, rsplen] & [epbsz, rsplen, 768hdd]
            # logits, sample_ids = self.decoder(knowledge_context_sentences, knowledge_context_encoded,
            #                                     response, response_embedding, training=True)
            if self.hparams.use_copy_decoder:   #   when pointer-generater, out-softmax.  Sequential cross-entropy loss for softmax output.     Only use this if you cannot get logits, otherwise use `tfa.seq2seq.SequenceLoss`
                response_label_smoothing = self.hparams.response_label_smoothing
                gen_loss = softmax_sequence_reconstruction_error(
                    logits, response[:, 1:], response_length - 1, average=True,
                    average_batch=False, smoothing_rate=response_label_smoothing,
                ...
                gen_loss = self.seq_loss(response[:, 1:], logits, response_mask[:, 1:])
            # gen_loss = tf.reduce_sum(tf.reshape(gen_loss, [batch_size, max_episode_length]), axis=1) \
            #     / tf.cast(episode_length, tf.float32)   # shape=(B,)
        else:                                   # here, we see the loss, hence not predict it
            # logits, sample_ids = self.decoder(knowledge_context_sentences, knowledge_context_encoded,
            #                                     response, response_embedding, training=True)
            if self.hparams.use_copy_decoder:   # my-note: when not training, we see the loss with no_label_smoothing
                response_label_smoothing = 0.0
                gen_loss = softmax_sequence_reconstruction_error(
                    logits, response[:, 1:], response_length - 1, average=True,
                    average_batch=False, smoothing_rate=response_label_smoothing,
                ...
                gen_loss = self.test_seq_loss(response[:, 1:], logits, response_mask[:, 1:])    # my-note: no_laebl_smoothing
            # gen_loss = tf.reduce_sum(tf.reshape(gen_loss, [batch_size, max_episode_length]), axis=1) \
            # / tf.cast(episode_length, tf.float32)
        # gen_loss = tf.reduce_sum(tf.reshape(gen_loss, [batch_size, max_episode_length]), axis=1) / tf.cast(episode_length, tf.float32)
        return gen_loss
...
class PredictiveLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, hparams, embedding, decoder, my_scope="my"):
        super().__init__()
    ...
    def call(self, Qprior, Xvec, Kvec, Knum, Xtensor, Xidx, Ytensor, Yidx, training=True):
        """
        ...
        Xtensor: float tensor [epbsz, xlen, hdddim]
        Xidx:   int tensor  [epbsz,xlen]
        """
        # NEAR_INF
        ...
        xxkk_mix = Xtensor + tf.expand_dims(chosen_knowledges, axis=1)
        """step 2. prediction """        
        # [batch_size, tgtlen-1, vocab_size] & [batch_size, tgtlen-1]
        pred_logits, pred_sample_ids = self.decoder(Xidx, xxkk_mix, Yidx, Ytensor, training=True)  # 
        if not training:
            pred_sample_ids, test_scores, max_test_output_length = self.decoder(Xidx, xxkk_mix, Yidx, Ytensor, training=False)

        if training:
            # reduce_logits = tf.reduce_max(pred_logits, axis=1)
            reduce_logits = tf.reduce_sum(pred_logits, axis=1)
            pred_vecs = tf.matmul(tf.nn.softmax(reduce_logits, axis=-1), self._embedding.shared_weights)
        else:
            reduce_logits = get_bow_from_seq(seq=pred_sample_ids, vocab_size=self.hparams.vocab_size, stop_commons=self.hparams.prediction_stop_common)
            pred_probs = tf.cast(soft_bow_to_distribution(reduce_logits), tf.float32)
            pred_vecs = tf.matmul(pred_probs, self._embedding.shared_weights)
